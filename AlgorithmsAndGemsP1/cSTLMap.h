...
Find People working
Using vector in list
Sort vector input
Load data into list with lsit or vector to list
SetContainerType
*/
...
{
public:
	cSTLMap() : containerType(eContainerType::STD_MAP) {}
	...
	std::map< int, sPerson > mapFemaleNames;
	std::map< int, sPerson > mapMaleNames;
	std::map< int, sPerson > mapLastNames;
	std::map< int, sPerson > mapGenerated;
	std::string error;
